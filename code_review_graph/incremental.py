...
def get_db_path(repo_root: Path) -> Path:
    ...
    Checks CRG_DB_PATH environment variable first, then falls back to
    .code-review-graph/graph.db in the repo root.
    """
    # Check environment variable override (useful for git worktrees)
    env_path = os.environ.get("CRG_DB_PATH")
    if env_path:
        p = Path(env_path)
        if p.suffix == ".db":
            p.parent.mkdir(parents=True, exist_ok=True)
            return p
        # If it's a directory, use graph.db inside it
    ...
    if not inner_gitignore.exists():
        inner_gitignore.write_text(
            "# Auto-generated by crg-jedi — do not commit database files.\n"
...
def full_build(repo_root: Path, store: GraphStore, jedi_resolver=None) -> dict:
    ...
    for i, rel_path in enumerate(files, 1):
        ...
        try:
            ...
            if jedi_resolver and parser.detect_language(full_path) == "python":
                edges = parser._enhance_calls_with_jedi(
                    full_path, source, nodes, edges, jedi_resolver
                )
...
def incremental_update(
    ...
    for rel_path in all_files:
        ...
        try:
            ...
            if jedi_resolver and parser.detect_language(abs_path) == "python":
                edges = parser._enhance_calls_with_jedi(
                    abs_path, source, nodes, edges, jedi_resolver
                )
